...
const (
	...
	ConsumerGroup = "social_insight_consumer"
	...
	// Batch config
	BatchSize     = 500             // Số posts mỗi batch insert
	FlushInterval = 2 * time.Second // Flush mỗi 2 giây
)
...
func main() {
	...
	handler := &PostHandler{
		redis:  redisClient,
		db:     db,
		buffer: make([]models.Post, 0, BatchSize),
	}
