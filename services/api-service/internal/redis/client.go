...
// SetLastCrawl lưu thời điểm crawl cuối cùng cho source
// key: last_crawl:{source} => RFC3339 timestamp
func (c *Client) SetLastCrawl(source string, t time.Time) error {
	key := fmt.Sprintf("last_crawl:%s", source)
	return c.rdb.Set(c.ctx, key, t.Format(time.RFC3339), 0).Err()
}

// GetLastCrawl lấy thời điểm crawl cuối cùng cho source
// Trả về zero time nếu không tồn tại
func (c *Client) GetLastCrawl(source string) (time.Time, error) {
	key := fmt.Sprintf("last_crawl:%s", source)
	s, err := c.rdb.Get(c.ctx, key).Result()
	if err != nil {
		if err == redis.Nil {
